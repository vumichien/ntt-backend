...
def import_csv(request):
    try:
        ...
        if not csv_file.name.endswith('.csv'):
            return Response({"error": "Đây không phải là tệp CSV"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "CSVファイルからデータが正常にインポートされました"}, status=status.HTTP_200_OK)
...
def import_error_log(request):
    try:
        ...
        for xlsx_file in xlsx_files:
            ...
            error_stats = defaultdict(lambda: {'count': 0, 'actions': [], 'users': set(), 'win_title': set()})
