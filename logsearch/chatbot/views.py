...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = r"C:\work\logsearch\ntt-backend\logsearch\logsearch-438406-3344f6159368.json"
...
def get_chat_response(request):
    ...
    if request.session.get('expecting_keyword', False):
        ...
        logs = MasterLogInfo.objects.filter(content__icontains=search_query).select_related('master_log')
        if logs.exists():
            ...
            get_questions_url = request.build_absolute_uri(reverse('get_questions', args=[log_id]))
            ...
            if response.status_code == 200:
                ...
                if questions:
                    ...
                    request.session['question_index'] = 0  # Lưu index của câu hỏi hiện tại
                    request.session['expecting_answer'] = True  # Đặt trạng thái đang đợi câu trả lời
            ...
            chatbot_message = "ログが見つかりませんでした。他のキーワードを試してください。"
    ...
    if request.session.get('expecting_answer', False):
        ...
        if question_index < len(questions) - 1:
            ...
            request.session['expecting_answer'] = False  # Kết thúc trạng thái đợi câu trả lời

            # Gọi API generate_procedure để tạo quy trình
            log_id = request.session.get('selected_log_id')
            generate_procedure_url = request.build_absolute_uri(reverse('generate_procedure', args=[log_id]))
            ...
            if response.status_code == 200:
                try:
                    ...
                    return JsonResponse({'message': chatbot_message, 'timeline': timeline_html})
